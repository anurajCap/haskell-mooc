...
firstHalf xs = take amount xs
  where
    amount = (length xs + 1) `div` 2
...
capitalize xs = capitalizeFirst first ++ (if not $ null rest then ' ' : capitalize rest else [])
  where
    first = head $ words xs
...
interpret x y [] = []
interpret x y ("up" : cs) = interpret x (y + 1) cs
interpret x y ("down" : cs) = interpret x (y - 1) cs
interpret x y ("left" : cs) = interpret (x - 1) y cs
interpret x y ("right" : cs) = interpret (x + 1) y cs
interpret x y ("printY" : cs) = show y : interpret x y cs
interpret x y ("printX" : cs) = show x : interpret x y cs
