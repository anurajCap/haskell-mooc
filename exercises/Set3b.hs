...
merge xss@(x : xs) yss@(y : ys) = if x >= y then y : merge xss ys else x : merge xs yss
...
mymaximum bigger initial (x : xs) = if initial `bigger` x then mymaximum bigger initial xs else mymaximum bigger x xs
